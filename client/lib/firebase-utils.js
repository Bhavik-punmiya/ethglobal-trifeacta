	...
	try {
		...
		const formattedData = {
			metadata: {
				...
				createdAt: serverTimestamp(), // Current server time
				status: "ongoing",
			...
			prizes: {
				totalPrizePool: Number.parseFloat(contestData.totalPrizePool),
				platformFee: 0.02, // 2% fixed fee
				winners: contestData.winners.map((winner) => ({
					place: winner.place,
					amount: Number.parseFloat(winner.amount),
					icon: winner.icon,
				})),
			},
			contract: {
				address: contestData.contractAddress,
				chain: "Ethereum", // Or your chosen blockchain
				chainId: 1, // Or your chosen chainId
